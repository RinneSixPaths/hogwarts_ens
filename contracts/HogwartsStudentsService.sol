...
contract HogwartsStudentsService is ERC721URIStorage {
  ...
  mapping (address => string) public addressesToDomains;
  ...
  function applyToHogwarts(string calldata name, string calldata patronus, uint nonce) public payable {
    ...
    string memory finalSvg = string(abi.encodePacked(svgBeforeColor, color, svgAfterColorBeforeText, domain, lastAfterText));
    ...
    addressesToDomains[msg.sender] = domain;
  ...
  function getRecordBySender() public view returns(HogwartsStudent memory) {
      string memory name = addressesToDomains[msg.sender];
